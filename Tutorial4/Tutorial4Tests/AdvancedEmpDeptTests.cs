...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e, d }).SelectMany(joined => grades.Where(g => joined.e.Sal >= g.Losal && joined.e.Sal <= g.Hisal).Select(g => new { EName = joined.e.EName, DName = joined.d.DName, Grade = g.Grade })).ToList(); 
